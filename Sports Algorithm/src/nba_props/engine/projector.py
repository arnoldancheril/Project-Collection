...
def project_player_stats(
    ...
    logs = _get_player_game_logs(conn, player_id, config.games_lookback)
    ...
    # Requested Weights: L5 (0.35), L20 (0.40), Season (0.25)
    def calculate_custom_weighted_average(values_list):
        ...
        if len(values_list) < 5:
            ...
            weighted_mean = (l5_avg * 0.40) + (season_avg * 0.60)
        else:
            weighted_mean = (l5_avg * 0.35) + (l20_avg * 0.40) + (season_avg * 0.25)
    ...
    if is_back_to_back:
        adj_factor *= config.back_to_back_factor
        adjustments["back_to_back"] = config.back_to_back_factor
    
    # Rest advantage
    if rest_days >= 3:
        adj_factor *= config.rest_advantage_factor
        adjustments["rest_advantage"] = config.rest_advantage_factor
    ...
    if config.use_archetype_adjustments and opponent_abbrev:
        try:
            ...
            if player_profile:
                ...
                if avoid:
                    archetype_adj *= config.elite_defender_factor
    ...
    adj_factor *= archetype_adj
    
    # Defense vs position adjustment (for OVER opportunities against weak defenses)
        ...
        try:
            ...
            for stat in ["pts", "reb", "ast"]:
                ...
                if defense_info:
                    ...
                    # Only apply boost for weak defenses (factor > 1.0)
                    # Scale the adjustment to be moderate: 
                    # - factor of 1.10 = defense allows 10% more -> 3-5% boost
                    # - factor of 1.20 = defense allows 20% more -> 6-10% boost
                    if factor > 1.02:  # Only if defense is clearly below average
                        # Apply a dampened adjustment (about 40-50% of the raw factor diff)
                        boost = 1.0 + (factor - 1.0) * 0.45
                        position_defense_adj[stat] = min(boost, 1.15)  # Cap at 15% boost
                        ...
                        if f"defense_vs_pos_{stat}" not in adjustments:
                            adjustments[f"defense_vs_pos_{stat}"] = {
                                ...
                                "rating": defense_info["rating"],
                                "boost_applied": position_defense_adj[stat],
                            }
        except ImportError:
            pass  # Defense position parser not available
    
    # Calculate final projections
    proj_minutes = avg_min * adj_factor
    proj_pts = avg_pts * adj_factor * position_defense_adj["pts"]
    proj_reb = avg_reb * adj_factor * position_defense_adj["reb"]
    proj_ast = avg_ast * adj_factor * position_defense_adj["ast"]
