...
class FilterPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.date_filter_active = False  # Track if user explicitly changed date range
        self.setup_ui()
        
    def setup_ui(self):
        """Set up the modern UI components"""
        # Fixed rail width, no horizontal scrolling; expand vertically
        ...
        # Set background color and modern styling for the entire panel
        self.setStyleSheet("""
            FilterPanel {
                background-color: #f8f9fa;
        ...
        self.company_input.setStyleSheet(f"""
            QLineEdit {{
        ...
        # Date Range card removed to condense filters (Time Frame covers it)
        ...
        self.apply_button.setStyleSheet(f"""
            ModernPillButton {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, 
                                          stop:0 {styles.COLORS['primary']}, stop:1 {styles.COLORS['primary_light']});
                color: white;
                border: none;
                border-radius: 25px;
                padding: 16px 32px;
            ...
            ModernPillButton:hover {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, 
                                          stop:0 {styles.COLORS['primary_dark']}, stop:1 {styles.COLORS['primary_light']});
            }}
            ModernPillButton:pressed {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, 
                                          stop:0 {styles.COLORS['primary_dark']}, stop:1 {styles.COLORS['primary']});
            }}
        """)
    ...
    def apply_filters(self):
        ...
        filters['apply_date'] = self.date_filter_active
        
        # Emit signal with filters
        self.filters_applied.emit(filters)

    def get_filters(self):
        """Return current filters dict, including whether date filter is active."""
        filters = {}
        company = self.company_input.text().strip()
        if company:
            filters['company'] = company
    ...
    def reset_filters(self):
        ...
        # Reset to all time by default and mark date as inactive
    ...
    def on_date_changed(self):
        """Mark date filter active and auto-apply when user changes dates."""
        self.date_filter_active = True
