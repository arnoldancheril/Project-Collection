...
def _resolve_team_name(text: str) -> Optional[str]:
    ...
    # Check partial matches (but only if text is reasonably long to avoid false matches)
    if len(text_lower) >= 3:
...
def parse_matchups_text(text: str) -> list[ParsedMatchup]:
    ...
    while i < len(lines):
        ...
        while j < len(lines) and j < i + 15:  # Look ahead up to 15 lines
            ...
            # Stop if we hit another team name (start of next matchup)
            if home_team and _resolve_team_name(next_line) and next_line.lower() not in line.lower():
                break
