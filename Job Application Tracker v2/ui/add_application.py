...
class AddApplicationDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Create header with gradient using stylesheet (more reliable)
        header = QWidget()
        ...
        # Title label with explicit white color and background
        ...
        location_label.setStyleSheet("font-weight: bold; color: #333;")
        
        # Get location suggestions and create auto-complete combo
        location_suggestions = self.get_location_suggestions()
        current_location = self.application.location if (self.is_edit_mode and self.application.location) else ""
        self.location_input = self.create_autocomplete_combo(
            location_suggestions, 
            "Enter location (City, State or Remote)...", 
            current_location
        )
        ...
        content_layout.addWidget(self.location_input)
    ...
    def get_location_suggestions(self):
        """Get location suggestions from common locations and database"""
        common_locations = [
            "Remote",
            "San Francisco, CA",
            "New York, NY", 
        ...
        suggestions = common_locations.copy()
        
        # Add frequently used locations from database if we have access
        if self.db_manager:
            try:
                # For now, we'll use a simple query since we don't have a dedicated method
                ...
                frequent_locations = sorted(location_counts.items(), key=lambda x: x[1], reverse=True)[:10]
                for location, _ in frequent_locations:
                    if location not in suggestions:
                        suggestions.append(location)
            except:
                # If there's any error, just use common locations
                pass
