...
def create_app() -> Flask:
    ...
    @app.route("/modellab")
    def modellab():
        """Model Lab page - comprehensive model analysis and backtesting."""
        return render_template("modellab_comprehensive.html")
    
    @app.route("/modellab-legacy")
    def modellab_legacy():
        """Legacy Model Lab page - original learning dashboard."""
        return render_template("modellab_new.html")
    
    @app.route("/lines")
    def lines_page():
        """Sportsbook lines page - view and fetch prop lines from The Odds API."""
        return render_template("lines.html")
    
    @app.route("/documentation")
    def documentation_page():
        """Documentation page - comprehensive guides for all models and features."""
    ...
    @app.route("/api/lines")
    def api_lines():
        """Get sportsbook lines for a specific date."""
        ...
        with db.connect() as conn:
            rows = conn.execute(
                """
                SELECT sl.id, sl.as_of_date, sl.prop_type, sl.line, 
                       sl.odds_american, sl.book,
        ...
        lines = []
        for r in rows:
            team_abbrev = abbrev_from_team_name(r["team_name"]) if r["team_name"] else None
            lines.append({
                "id": r["id"],
        ...
        return jsonify({"lines": lines, "date": date})
    
    @app.route("/api/lines/status")
    def api_lines_status():
        """Get API status and line counts."""
        db = get_db()
        
        # Get total lines count and latest date
        with db.connect() as conn:
            total = conn.execute("SELECT COUNT(*) AS n FROM sportsbook_lines").fetchone()["n"]
            latest = conn.execute(
                "SELECT as_of_date FROM sportsbook_lines ORDER BY created_at DESC LIMIT 1"
            ).fetchone()
        
        # Try to get API usage from the odds_api_client
        try:
            ...
            if response.success and response.usage:
                return jsonify({
                    ...
                    "requests_limit": 500,  # The Odds API free tier limit
                    "total_lines": total,
                    "last_fetch": latest["as_of_date"] if latest else None,
                })
        except Exception:
            pass
    ...
    @app.route("/api/lines/fetch", methods=["POST"])
    def api_fetch_lines():
        """Fetch new lines from The Odds API."""
        try:
            from ..ingest.odds_api_client import (
                fetch_all_player_props_for_date,
                store_player_props,
                get_api_usage,
                DEFAULT_API_KEY,
            )
            ...
            markets = data.get('markets', ['player_points', 'player_rebounds'])
            
            # Fetch props from API - returns tuple (props_list, usage_info)
            props_list, usage_info = fetch_all_player_props_for_date(
                date=date_str,
                markets=markets,
                verbose=True,
            )
            ...
            if not props_list:
                return jsonify({"error": "No props found for the specified date. There may be no games scheduled."}), 400
            
            # Store in database
            db = get_db()
            with db.connect() as conn:
                stored_count = store_player_props(conn, props_list, as_of_date=date_str)
                conn.commit()
            
            # Get updated API usage
            requests_remaining = None
            if usage_info:
                requests_remaining = usage_info.requests_remaining
            
            return jsonify({
                "success": True,
                "props_fetched": len(props_list),
                "props_stored": stored_count,
                "requests_remaining": requests_remaining,
            })
        except ImportError as e:
            return jsonify({"error": f"API client not available: {str(e)}"}), 500
        except Exception as e:
            import traceback
            traceback.print_exc()
    ...
    def api_matchup_analysis():
        ...
        try:
            with db.connect() as conn:
                ...
                # Generate V16 picks for this specific matchup
                v16_plays = []
                try:
                    from ..engine.model_v16_general import get_daily_picks_v16_general
                    from ..engine.model_v16_under import get_daily_picks_v16_under
                    
                    # Get V16 General picks
                    general_picks = get_daily_picks_v16_general(game_date)
                    # Get V16 Under picks
                    under_picks = get_daily_picks_v16_under(game_date)
                    
                    # Filter for this matchup only
                    all_v16_picks = general_picks.picks + under_picks.picks
                    matchup_picks = [
                        p for p in all_v16_picks 
                        if (p.team_abbrev in [away, home] or p.opponent_abbrev in [away, home])
                    ]
                    
                    for pick in matchup_picks:
                        model_source = "V16 General" if pick in general_picks.picks else "V16 Under"
                        # Handle different attribute names between V16 General and V16 Under
                        adjusted_proj = getattr(pick, 'projection_adj', None) or getattr(pick, 'adjusted_projection', 0.0)
                        baseline = getattr(pick, 'season_avg', 0.0)
                        v16_plays.append({
                            ...
                            "line": pick.line,
                            "adjustment_pct": round((adjusted_proj - baseline) / baseline * 100, 1) if baseline > 0 else 0,
                            "confidence": pick.confidence_tier,
                            "confidence_score": pick.confidence_score,
                            "reasons": pick.reasons[:5],
                            "warnings": [],
                            "source": model_source,
                            "pattern": getattr(pick, 'pattern', 'factor_based'),
                            "edge_pct": pick.edge_pct,
                        })
                except Exception as e:
                    import traceback
                    print(f"Note: V16 models not available for matchup: {e}")
                    traceback.print_exc()
                ...
                # Generate V2 Multi-Model picks for this matchup
                v2_plays = []
                try:
                    from ..engine.multi_model_picker import get_multi_model_picks
                    v2_daily = get_multi_model_picks(game_date)
                    
                    # Team name mapping for filtering (abbrev -> full name patterns)
                    team_patterns = {
                        ...
                        "UTA": ["Jazz", "Utah Jazz"],
                        "MIN": ["Wolves", "Timberwolves", "Minnesota Timberwolves"],
                    ...
                    away_patterns = team_patterns.get(away, [away])
                    home_patterns = team_patterns.get(home, [home])
                    
                    # Filter for this matchup
                    for pick in v2_daily.picks:
                        team_matches = any(p in pick.team for p in away_patterns + home_patterns) or pick.team in [away, home]
                        opp_matches = any(p in pick.opponent for p in away_patterns + home_patterns) or pick.opponent in [away, home]
                        
                        if team_matches or opp_matches:
                            v2_plays.append({
                                "player": pick.player_name,
                                "team": pick.team,
                                ...
                                "direction": pick.direction,
                                "line": pick.line,
                                "baseline": pick.projection,  # Using projection as baseline
                                "adjusted": pick.projection,
                                "adjustment_pct": pick.edge,
                                "confidence": pick.tier.display if hasattr(pick.tier, 'display') else str(pick.tier),
                                "confidence_score": pick.confidence_score,
                                "reasons": pick.key_factors[:5] if pick.key_factors else [],
                                "warnings": [],
                                "source": "V2 System",
                    ...
                    import traceback
                    print(f"Note: V2 multi_model_picker not available: {e}")
                    traceback.print_exc()
            ...
            result = {
                ...
                "best_over_plays": (
                    # Combine traditional over plays with V16 over plays and V2 plays
                    [transform_matchup_edge(e) for e in report.best_over_plays] +
                    [p for p in v16_plays if p.get("direction") == "OVER"] +
                    [p for p in v2_plays if p.get("direction") == "OVER"]
                ),
                "best_under_plays": (
                    # Combine traditional under plays with under_v2 plays, V16 under plays, and V2 plays
                    [transform_matchup_edge(e) for e in report.best_under_plays] + 
                    under_v2_plays +
                    [p for p in v16_plays if p.get("direction") == "UNDER"] +
                    [p for p in v2_plays if p.get("direction") == "UNDER"]
                ),
                "v16_picks": v16_plays,  # Include all V16 picks separately too
                "v2_picks": v2_plays,  # Include V2 picks separately
                "avoid_players": report.avoid_players,
                "key_matchups": report.key_matchups,
            }
            
            # Sort over plays by confidence score
            result["best_over_plays"] = sorted(
    ...
    def api_injuries():
        """
        Get injury report. Supports two modes:
        
        Mode 1 (General Query): 
            GET /api/injuries?date=YYYY-MM-DD&team=TeamName
            Returns list of all injuries matching filters
            
        Mode 2 (Matchup Query):
            GET /api/injuries?away=LAL&home=BOS&date=YYYY-MM-DD
            Returns injuries grouped by away/home team for matchup analysis
        """
        date = request.args.get("date", "")
        team = request.args.get("team", "")
        ...
        # Mode 2: Matchup query (when away/home params are provided)
        if away or home:
            game_date = date or datetime.now().strftime("%Y-%m-%d")
            try:
                with db.connect() as conn:
                    # Get away team injuries
                    away_injuries = {}
                    if away:
                        # Try by abbrev first, then by name lookup
                        away_team_id = conn.execute(
                            "SELECT id FROM teams WHERE abbrev = ?", (away,)
                        ).fetchone()
                        if not away_team_id:
                            # Try to find by name containing the abbrev
                            away_team_id = conn.execute(
                                "SELECT id FROM teams WHERE name LIKE ?", (f"%{away}%",)
                            ).fetchone()
                        if away_team_id:
                            rows = conn.execute(
                                """
                                ...
                                WHERE ir.team_id = ? AND ir.game_date = ?
                                """,
                    ...
                    if home:
                        ...
                        if not home_team_id:
                            home_team_id = conn.execute(
                                "SELECT id FROM teams WHERE name LIKE ?", (f"%{home}%",)
                            ).fetchone()
                        if home_team_id:
                            rows = conn.execute(
                                """
    ...
    # =========================================================================
    # Comprehensive Model Lab - All Models Backtesting System
    # =========================================================================
    
    @app.route("/api/modellab/models")
    def api_modellab_get_models():
        """
        Get list of all registered models with their capabilities.
        """
        try:
            from ..engine.model_registry import (
                get_all_models, get_model_count_summary
            )
                ...
                "models": [
                    {
                        ...
                        "primary_file": m.primary_file,
                        "secondary_files": m.secondary_files,
                    }
                    for m in models
                ],
                "summary": summary,
    ...
    @app.route("/api/modellab/comprehensive-backtest", methods=["POST"])
    def api_modellab_comprehensive_backtest():
        """
        Run comprehensive backtest across multiple models.
        
        Request body:
            - start_date: Start date (YYYY-MM-DD)
            - end_date: End date (YYYY-MM-DD)
            - model_ids: Optional list of specific model IDs to test
            - category: Optional category filter ("single_file", "multi_file", "specialized")
        
        Returns detailed results for all tested models with rankings.
        """
        try:
            ...
            start_date = data.get("start_date")
            end_date = data.get("end_date")
            model_ids = data.get("model_ids")  # Optional list of specific models
            category = data.get("category")  # Optional category filter
            
            if not start_date or not end_date:
                return jsonify({"error": "Missing start_date or end_date"}), 400
            
            # If category specified, get models of that category
            if category and not model_ids:
                from ..engine.model_registry import (
                    get_models_by_category, ModelCategory
                )
                try:
                    cat_enum = ModelCategory(category)
                    cat_models = get_models_by_category(cat_enum)
                    model_ids = [m.model_id for m in cat_models if m.is_active]
                except ValueError:
                    pass  # Invalid category, test all
            ...
            # Get recommendations
            recommendations = get_model_recommendations(results)
    ...
    @app.route("/api/modellab/single-backtest", methods=["POST"])
    def api_modellab_single_backtest():
        """
        ...
        Returns detailed results for the specified model.
        """
        try:
            from ..engine.comprehensive_backtester import run_single_model_backtest
            ...
            if result is None:
                return jsonify({
                    "success": False,
                    "error": f"Model '{model_id}' not found or backtest failed"
                }), 404
    ...
    @app.route("/api/modellab/compare-latest", methods=["POST"])
    def api_modellab_compare_latest():
        """
    ...
    @app.route("/api/modellab/compare-under", methods=["POST"])
    def api_modellab_compare_under():
        """
        Compare all UNDER-specialized models.
    ...
    @app.route("/api/modellab/model-info/<model_id>")
    def api_modellab_model_info(model_id: str):
        """
        Get detailed information about a specific model.
        """
        try:
            from ..engine.model_registry import get_model_by_id
            ...
            return jsonify({
                "model": {
                    ...
                    "primary_file": model.primary_file,
                    "secondary_files": model.secondary_files,
                    "backtest_function": model.backtest_function,
                    "picks_function": model.picks_function,
    ...
    def _generate_v10_picks(target_date: str, force: bool = False):
        """
        Helper function to generate picks using Model V10 (Market-Aware).
        
        Model V10 is the recommended model that:
        - REQUIRES actual sportsbook lines (no line = no pick)
        - Uses strategic direction selection (PTS UNDER preferred)
        - Excludes AST props (too volatile ~54%)
        - Requires pattern confirmation for all picks
        - Integrates elite defense from DVP data
        
        Backtest: 61.1% overall, 67.2% PREMIUM tier
        """
        import json
        from ..engine.model_v10 import get_daily_picks_v10, ModelConfigV10
        
        db = get_db()
        
        try:
            with db.connect() as conn:
                # Check for cached v10 picks first
                if not force:
                    cached = conn.execute(
                        """
                        SELECT p.*, r.actual_value, r.hit, r.margin
                    ...
                    if cached:
                        ...
                        for row in cached:
                            ...
                            picks.append({
                                ...
                                "confidence_score": row["confidence_score"],
                                "confidence_stars": stars,
                                "reasons": [r for r in reasons if r != "[model_v10]"],
                                "rank": row["rank"],
                                "actual": row["actual_value"],
                                "hit": row["hit"],
                                "result": row["hit"] is not None,
                ...
                # Allow derived lines for testing when sportsbook lines unavailable
                config.require_sportsbook_line = False
                
                # Generate fresh picks using Model V10
                daily = get_daily_picks_v10(target_date, config=config)
                ...
                if not daily.picks:
                    return jsonify({
                        ...
                        "date": target_date,
                        "model": "v10",
                        "message": f"No picks found - {daily.players_without_lines} players had no lines",
                        "games_count": daily.games,
                        "skipped_no_line": daily.players_without_lines,
                    })
                
                # Clear old v10 picks if force
                if force:
                    old_ids = [r["id"] for r in conn.execute(
                        """
                        SELECT id FROM model_picks 
                        WHERE pick_date = ? 
                          AND reasons LIKE '%model_v10%'
                        """, 
                        (target_date,)
                    ).fetchall()]
                    if old_ids:
                ...
                for rank, pick in enumerate(daily.picks, 1):
                    # Build reasons list with model marker
                    reasons_list = pick.reasons[:5] + ["[model_v10]"]
                    
                    # Map confidence tier
                    confidence_tier = pick.confidence_tier  # PREMIUM, HIGH, STANDARD
                    ...
                    picks_response.append({
                        ...
                        "prop": pick.prop_type,
                        "direction": pick.direction,
                        "projection": round(pick.projection, 1),
                        "line": round(pick.sportsbook_line, 1),
                        "confidence": confidence_tier,
                        "confidence_score": round(pick.confidence_score, 1),
                        "confidence_stars": stars,
                        "is_star_player": False,
                        "has_h2h": False,
                        "is_b2b": False,
                        ...
                        "result": False,
                        # V10 specific info
                        "edge": round(pick.edge_vs_line, 1),
                        "pattern": pick.pattern,
                        "defense_rating": pick.defense_rating,
                        "line_source": pick.book or "derived",
                    })
                
                conn.commit()
                
                # Calculate stats
                premium_count = sum(1 for p in picks_response if p["confidence"] == "PREMIUM")
                high_count = sum(1 for p in picks_response if p["confidence"] == "HIGH")
                under_count = sum(1 for p in picks_response if p["direction"] == "UNDER")
                ...
                return jsonify({
                    ...
                    "model": "v10",
                    "total_picks": len(picks_response),
                    "premium_picks": premium_count,
                    "high_picks": high_count,
                    "under_picks": under_count,
                    "over_picks": len(picks_response) - under_count,
                    "games_count": daily.games,
                    "skipped_no_line": daily.players_without_lines,
                })
    ...
    def _generate_v16_general_picks(target_date: str, force: bool = False):
        """
        Helper function to generate picks using Model V16 (Dual-Model Approach).
        
        This function runs BOTH the V16 General and V16 Under models and combines picks:
        - V16 General: Pattern-based picks (Cold Bounce, B2B Fatigue, Elite Defense)
        - V16 Under: Specialized UNDER analysis with factor-based scoring
        
        Under picks from V16 General are verified/supplemented with V16 Under model.
        
        Backtest: V16 General 72.4%, V16 Under PREMIUM 84.6%
        """
        import json
        from ..engine.model_v16_general import get_daily_picks_v16_general, ModelConfigV16General
        from ..engine.model_v16_under import get_daily_picks_v16_under, ModelConfigV16Under
        
        db = get_db()
        
        try:
            with db.connect() as conn:
                # Check for cached v16 picks first (both general and under)
                if not force:
                    cached = conn.execute(
                        """
                        SELECT p.*, r.actual_value, r.hit, r.margin
                        ...
                        LEFT JOIN model_pick_results r ON r.pick_id = p.id
                        WHERE p.pick_date = ?
                          AND (p.reasons LIKE '%model_v16_general%' OR p.reasons LIKE '%model_v16_under%')
                        ORDER BY p.confidence_score DESC, p.rank
                        """,
                        (target_date,),
                    ).fetchall()
                    ...
                    if cached:
                        ...
                        for row in cached:
                            ...
                            # Determine model source from reasons
                            model_source = "V16 General"
                            if "[model_v16_under]" in reasons:
                                model_source = "V16 Under"
                            ...
                            picks.append({
                                ...
                                "confidence_score": row["confidence_score"],
                                "confidence_stars": stars,
                                "reasons": [r for r in reasons if not r.startswith("[model_v16")],
                                "rank": row["rank"],
                                "actual": row["actual_value"],
                                "hit": row["hit"],
                                "result": row["hit"] is not None,
                ...
                # Configure Model V16 General
                config_general = ModelConfigV16General()
                
                # Generate fresh picks using Model V16 General
                daily_general = get_daily_picks_v16_general(target_date, config=config_general)
                
                # ALSO run V16 Under model to supplement/verify under picks
                config_under = ModelConfigV16Under()
                daily_under = get_daily_picks_v16_under(target_date, config=config_under)
                
                # Track which players already have picks from general model
                general_players = set()
                for pick in daily_general.picks:
                    general_players.add((pick.player_name, pick.prop_type))
                
                if not daily_general.picks and not daily_under.picks:
                    return jsonify({
                        "picks": [],
                        "cached": False,
                        "date": target_date,
                        "model": "v16_general",
                        "message": f"No picks found for {target_date}",
                        "games_count": daily_general.games,
                    })
                
                # Clear old v16 picks (both general and under) if force
                if force:
                    old_ids = [r["id"] for r in conn.execute(
                        """
                        SELECT id FROM model_picks 
                        WHERE pick_date = ? 
                          AND (reasons LIKE '%model_v16_general%' OR reasons LIKE '%model_v16_under%')
                        """, 
                        (target_date,)
                    ).fetchall()]
                    if old_ids:
                ...
                # First add V16 General picks
                for rank, pick in enumerate(daily_general.picks, 1):
                    # Build reasons list with model marker
                    reasons_list = pick.reasons[:5] + ["[model_v16_general]"]
                    ...
                    picks_response.append({
                        ...
                        "prop": pick.prop_type,
                        "direction": pick.direction,
                        "projection": round(pick.projection_adj, 1),
                ...
                # Now add V16 Under picks that are NOT already in the response
                # (avoiding duplicates for same player/prop)
                under_start_rank = len(picks_response) + 1
                for pick in daily_under.picks:
                    player_prop_key = (pick.player_name, pick.prop_type)
                    if player_prop_key not in general_players:
                        # Build reasons list with model marker
                        reasons_list = pick.reasons[:5] + ["[model_v16_under]"]
                        ...
                        conn.execute(
                            ...
                            (
                                ...
                                pick.opponent_abbrev,
                                pick.prop_type,
                                "UNDER",  # Always UNDER for this model
                                pick.adjusted_projection,
                                pick.line,
                                pick.confidence_tier,
                                pick.confidence_score,
                                json.dumps(reasons_list),
                        ...
                        factor_names = [f.name for f in pick.factors[:3]] if pick.factors else []
                        ...
                        picks_response.append({
                            ...
                            "prop": pick.prop_type,
                            "direction": "UNDER",
                            "projection": round(pick.adjusted_projection, 1),
                            "line": round(pick.line, 1),
                            "confidence": pick.confidence_tier,
                            "confidence_score": round(pick.confidence_score, 1),
                            "confidence_stars": stars,
                            ...
                            "is_b2b": pick.is_b2b,
                            "reasons": pick.reasons[:5],
                            "rank": under_start_rank,
                            "actual": None,
                            "hit": None,
                            "result": False,
                            # V16 Under specific info
                            "edge": round(pick.edge_pct, 1),
                            "factor_score": round(pick.factor_score, 1),
                            "factors": factor_names,
                            "defense_rating": pick.defense_rating,
                            "defense_rank": pick.defense_rank,
                            "line_source": pick.line_source,
                            "cold_streak": pick.cold_streak_severity,
                            "model_source": "V16 Under",
                        })
                        under_start_rank += 1
                
                # Sort all picks by confidence_score descending
                picks_response.sort(key=lambda p: p["confidence_score"], reverse=True)
                
                # Re-assign ranks after sorting
                for i, pick in enumerate(picks_response, 1):
                    pick["rank"] = i
                ...
                high_count = sum(1 for p in picks_response if p["confidence"] == "HIGH")
                under_count = sum(1 for p in picks_response if p["direction"] == "UNDER")
                v16_general_count = sum(1 for p in picks_response if p.get("model_source") == "V16 General")
                v16_under_count = sum(1 for p in picks_response if p.get("model_source") == "V16 Under")
                ...
                return jsonify({
                    ...
                    "over_picks": len(picks_response) - under_count,
                    "games_count": daily_general.games,
                    "sportsbook_picks": len(daily_general.sportsbook_picks) + len(daily_under.sportsbook_picks),
                    "derived_picks": len(daily_general.derived_picks) + len(daily_under.derived_picks),
                    "v16_general_picks": v16_general_count,
                    "v16_under_picks": v16_under_count,
                })
    ...
    def _generate_v16_under_picks(target_date: str, force: bool = False):
        """
        Helper function to generate picks using Model V16.5 Under.
        
        Model V16.5 Under is the SPECIALIZED UNDER model that:
        - Factor-based scoring (defense_elite +30, cold_streak_severe +22, b2b +8)
        - PREMIUM tier (score ≥50) hits at 84.6%
        - Elite Defense + Cold Streak combined = 83.3%
        - PTS UNDER only (REB disabled - too volatile)
        - Hybrid line approach
        
        Backtest: 60.6% overall (103/170), PREMIUM 84.6% (11/13)
        """
        import json
        from ..engine.model_v16_under import get_daily_picks_v16_under, ModelConfigV16Under
        ...
        try:
            with db.connect() as conn:
                # Check for cached v16_under picks first
                if not force:
                    cached = conn.execute(
                        """
                        SELECT p.*, r.actual_value, r.hit, r.margin
                        ...
                        LEFT JOIN model_pick_results r ON r.pick_id = p.id
                        WHERE p.pick_date = ?
                          AND p.reasons LIKE '%model_v16_under%'
                        ORDER BY p.confidence_score DESC, p.rank
                        """,
                        (target_date,),
                    ).fetchall()
                    ...
                    if cached:
                        ...
                        for row in cached:
                            ...
                            picks.append({
                                ...
                                "confidence_score": row["confidence_score"],
                                "confidence_stars": stars,
                                "reasons": [r for r in reasons if r != "[model_v16_under]"],
                                "rank": row["rank"],
                                "actual": row["actual_value"],
                                "hit": row["hit"],
                                "result": row["hit"] is not None,
                ...
                # Configure Model V16.5 Under
                config = ModelConfigV16Under()
                
                # Generate fresh picks using Model V16.5 Under
                daily = get_daily_picks_v16_under(target_date, config=config)
                ...
                if not daily.picks:
                    return jsonify({
                        ...
                        "date": target_date,
                        "model": "v16_under",
                        "message": f"No UNDER picks found for {target_date}",
                        "games_count": daily.games,
                    })
                
                # Clear old v16_under picks if force
                if force:
                    old_ids = [r["id"] for r in conn.execute(
                        """
                        SELECT id FROM model_picks 
                        WHERE pick_date = ? 
                          AND reasons LIKE '%model_v16_under%'
                        """, 
                        (target_date,)
                    ).fetchall()]
                    if old_ids:
                ...
                for rank, pick in enumerate(daily.picks, 1):
                    ...
                    # Build factors display
                    factor_names = [f.name for f in pick.factors[:3]]
                ...
                # Calculate stats
                premium_count = len(daily.premium_picks)
                high_count = len(daily.high_picks)
                ...
                return jsonify({
                    ...
                    "premium_picks": premium_count,
                    "high_picks": high_count,
                    "under_picks": len(picks_response),  # All are UNDER
                    "over_picks": 0,
                    "games_count": daily.games,
                    "sportsbook_picks": len(daily.sportsbook_picks),
                    "derived_picks": len(daily.derived_picks),
                })
    ...
    def api_backtesting_generate_picks():
        ...
        model_version = data.get("model", "v16_general")
        
        # Handle V16 General model (RECOMMENDED - 72.4% hit rate)
        if model_version == "v16_general":
            return _generate_v16_general_picks(target_date, force)
        
        # Handle V16.5 Under model (UNDER specialist - 60.6%, PREMIUM 84.6%)
        if model_version == "v16_under":
            return _generate_v16_under_picks(target_date, force)
        ...
        # Handle V10 model separately (market-aware with sportsbook lines)
        if model_version == "v10":
            return _generate_v10_picks(target_date, force)
    ...
    def api_backtesting_compare_results():
        ...
        Request body:
            ...
            - model: Model to filter by (optional, defaults to v16_general which includes v16_under)
        ...
        data = request.json or {}
        target_date = data.get("date", datetime.now().strftime("%Y-%m-%d"))
        model = data.get("model", "v16_general")
        
        db = get_db()
        
        try:
            with db.connect() as conn:
                # Get picks for the date - FILTER BY MODEL
                # For v16_general, include both v16_general and v16_under picks
                if model == "v16_general":
                    picks = conn.execute(
                        """
                    ...
                    # For other models, try to filter by model marker
                    model_marker = f"[{model}]" if not model.startswith("[") else model
                    picks = conn.execute(
                        """
                        SELECT p.*, r.actual_value, r.hit
                        FROM model_picks p
                        ...
                        ORDER BY p.confidence_score DESC, p.rank
                        """,
                        (target_date, f"%{model_marker}%"),
                    ).fetchall()
                
                if not picks:
                    return jsonify({"error": f"No picks found for {target_date} with model {model}. Generate picks first."}), 404
                ...
                for pick in picks:
                    ...
                    if "[model_v16_general]" in reasons:
                        model_source = "V16 General"
                    elif "[model_v16_under]" in reasons:
                        model_source = "V16 Under"
                    elif "[under_model_v2]" in reasons:
                        model_source = "Under V2"
                    elif "[model_v10]" in reasons:
                        model_source = "V10"
                    ...
                    results.append({
                        ...
                        "confidence_score": pick["confidence_score"],
                        "reasons": [r for r in reasons if not r.startswith("[model_") and not r.startswith("[under_")],
                        "rank": pick["rank"],
                        "actual": actual_value,
                        "hit": hit,
    ...
    # -------------------------------------------------------------------------
    # Save Scraped Data to Database Endpoints
    # -------------------------------------------------------------------------
    
    @app.route("/api/scrape/injuries/save", methods=["POST"])
    def api_save_scraped_injuries():
        """
        Save scraped injuries to the database.
        
        Request body:
            - injuries: List of injury objects from scrape result
            - date: Optional date to associate with injuries (defaults to today)
        """
        from ..db import get_or_create_team
        import unicodedata
        ...
        game_date = data.get("date", datetime.now().strftime("%Y-%m-%d"))
        
        if not injuries:
            return jsonify({"error": "No injuries data provided"}), 400
        ...
        try:
            ...
            with db.connect() as conn:
                ...
                for injury in injuries:
                    ...
                    # ESPN uses: "Out", "Day-To-Day", "Suspension", "Injured Reserve", etc.
                    status_upper = status.upper()
                    if "OUT" in status_upper or "SUSPENSION" in status_upper or "INJURED RESERVE" in status_upper:
                        mapped_status = "OUT"
                    elif "DOUBTFUL" in status_upper:
                        mapped_status = "DOUBTFUL"
                    elif "QUESTIONABLE" in status_upper or "DAY-TO-DAY" in status_upper or "GTD" in status_upper:
                        mapped_status = "QUESTIONABLE"
                    elif "PROBABLE" in status_upper:
                        mapped_status = "PROBABLE"
                    elif "AVAILABLE" in status_upper or "ACTIVE" in status_upper:
                        # Skip players who are available (no need to store them)
                        continue
                    else:
                        # Default to OUT for unknown statuses (safer for betting decisions)
                        mapped_status = "OUT"
                    
                    team_id = get_or_create_team(conn, team_name)
                    
                    # Try to find existing player
                    normalized_name = normalize_name_for_match(player_name)
                    player_id = player_lookup.get(normalized_name)
                    
                    if not player_id:
                        for normalized_db_name, pid in player_lookup.items():
                            if (normalized_name in normalized_db_name or 
                                normalized_db_name in normalized_name):
                                player_id = pid
                                break
                    
                    # Check if entry already exists
                    existing = conn.execute(
                        "SELECT id FROM injury_report WHERE game_date = ? AND player_name = ?",
                    ...
                    if existing:
                        conn.execute(
                            ...
                            SET status = ?, notes = ?, player_id = ?, team_id = ?
                            WHERE id = ?""",
                            (mapped_status, comment, player_id, team_id, existing["id"]),
                        )
                        updated_count += 1
                    else:
                        conn.execute(
                            ...
                            (game_date, team_id, player_id, player_name, status, notes)
                            VALUES (?, ?, ?, ?, ?, ?)""",
                            (game_date, team_id, player_id, player_name, mapped_status, comment),
                        )
                        saved_count += 1
    ...
    @app.route("/api/scrape/matchups/save", methods=["POST"])
    def api_save_scraped_matchups():
        """
        Save scraped matchups to the database.
        
        Request body:
            - matchups: List of matchup objects from scrape result
        """
        from ..db import get_or_create_team
        ...
        if not matchups:
            return jsonify({"error": "No matchups data provided"}), 400
            ...
            with db.connect() as conn:
                for m in matchups:
                    ...
                    existing = conn.execute("""
                        SELECT id FROM scheduled_games 
                        WHERE game_date = ? AND away_team_id = ? AND home_team_id = ?
                    """, (game_date, away_team_id, home_team_id)).fetchone()
                    
                    if existing:
                        conn.execute("""
                            UPDATE scheduled_games 
                            SET spread = ?, over_under = ?, game_time = ?, tv_channel = ?, 
                                status = ?, updated_at = datetime('now')
                            WHERE id = ?
                        """, (spread, over_under, game_time, tv_channel, status, existing["id"]))
                        updated_count += 1
                    else:
                        conn.execute("""
                            INSERT INTO scheduled_games 
                            (game_date, away_team_id, home_team_id, spread, over_under, game_time, tv_channel, status)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (game_date, away_team_id, home_team_id, spread, over_under, game_time, tv_channel, status))
                        saved_count += 1
    ...
    @app.route("/api/scrape/boxscores/save", methods=["POST"])
    def api_save_scraped_boxscores():
        """
        Save scraped box scores to the database.
        
        Request body:
            - box_scores: List of box score objects from scrape result
        """
        from ..db import get_or_create_team, get_or_create_player
        ...
        if not box_scores:
            return jsonify({"error": "No box scores data provided"}), 400
        ...
        try:
            ...
            with db.connect() as conn:
                for bs in box_scores:
                    ...
                    game_date = bs.get("game_date", "")
                    away_score = bs.get("away_score")
                    home_score = bs.get("home_score")
                    player_stats = bs.get("player_stats", [])
                    ...
                    """, (game_date, away_team_id, home_team_id, home_team_id, away_team_id)).fetchone()
                    ...
                    if existing_game:
                        ...
                        cur = conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?)
                        """, (season, game_date, away_team_id, home_team_id, "web_scraper", f"scraped_{game_date}"))
                    ...
                    if away_score is not None:
                        existing_totals = conn.execute(
                            "SELECT id FROM boxscore_team_totals WHERE game_id = ? AND team_id = ?",
                            (game_id, away_team_id)
                        ).fetchone()
                        if existing_totals:
                            conn.execute(
                                "UPDATE boxscore_team_totals SET pts = ? WHERE id = ?",
                                (away_score, existing_totals["id"])
                            )
                        else:
                            conn.execute("""
                                INSERT INTO boxscore_team_totals (game_id, team_id, pts)
                                VALUES (?, ?, ?)
                            """, (game_id, away_team_id, away_score))
                    ...
                    if home_score is not None:
                        ...
                        if existing_totals:
                            conn.execute(
                                "UPDATE boxscore_team_totals SET pts = ? WHERE id = ?",
                                (home_score, existing_totals["id"])
                            )
                        else:
                            conn.execute("""
                                INSERT INTO boxscore_team_totals (game_id, team_id, pts)
                                VALUES (?, ?, ?)
                            """, (game_id, home_team_id, home_score))
                    ...
                    for ps in player_stats:
                        ...
                        minutes_str = ps.get("minutes", "0:00")
                        minutes = None
                        ...
                        # Parse FG, 3P, FT strings (e.g., "5-10")
                        ...
                        fgm, fga = parse_made_attempted(ps.get("fg"))
                        tpm, tpa = parse_made_attempted(ps.get("tp"))
                        ftm, fta = parse_made_attempted(ps.get("ft"))
                        ...
                        blk = ps.get("blk")
                        tov = ps.get("tov")
                        plus_minus = ps.get("plus_minus")
                        
                        # Check if player stat already exists
                        existing_ps = conn.execute("""
                            SELECT id FROM boxscore_player 
                            WHERE game_id = ? AND player_id = ?
                        """, (game_id, player_id)).fetchone()
                        
                        if existing_ps:
                            conn.execute("""
                                UPDATE boxscore_player SET
                                    team_id = ?, status = 'PLAYED', minutes = ?,
                                    pts = ?, reb = ?, ast = ?, stl = ?, blk = ?, tov = ?,
                                    fgm = ?, fga = ?, tpm = ?, tpa = ?, ftm = ?, fta = ?,
                                    plus_minus = ?
                                WHERE id = ?
                            """, (team_id, minutes, pts, reb, ast, stl, blk, tov,
                                  fgm, fga, tpm, tpa, ftm, fta, plus_minus, existing_ps["id"]))
                        else:
                            conn.execute("""
                                INSERT INTO boxscore_player 
                                (game_id, team_id, player_id, status, minutes, pts, reb, ast, 
                                 stl, blk, tov, fgm, fga, tpm, tpa, ftm, fta, plus_minus)
                                VALUES (?, ?, ?, 'PLAYED', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """, (game_id, team_id, player_id, minutes, pts, reb, ast,
                                  stl, blk, tov, fgm, fga, tpm, tpa, ftm, fta, plus_minus))
                            players_saved += 1
    ...
    # =========================================================================
    # V2 SYSTEM API - Multi-Model Pattern-Based Picker
    # =========================================================================
    
    @app.route("/api/v2-system/picks", methods=["POST"])
    def api_v2_system_picks():
        """
        Generate V2 picks for a specific date.
        
        Request body:
            - date: Target date (YYYY-MM-DD)
        
        Returns daily picks with pattern information.
        """
        try:
            from ..engine.multi_model_picker import get_multi_model_picks
            ...
            daily_picks = get_multi_model_picks(target_date)
            
            # Convert picks to JSON-serializable format
            picks_list = []
            for pick in daily_picks.picks:
                picks_list.append({
                    ...
                    "direction": pick.direction,
                    "line": pick.line,
                    "line_source": pick.line_source.value if hasattr(pick.line_source, 'value') else str(pick.line_source),
                    "projection": pick.projection,
                    "edge": pick.edge,
                    "confidence": pick.confidence_score,
                    "tier": pick.tier.display if hasattr(pick.tier, 'display') else str(pick.tier),
                    "pattern": pick.primary_pattern,
                    "factors": pick.key_factors,
                    "actual": pick.actual_value,
                    "result": pick.hit,
                })
            
            return jsonify({
                "date": daily_picks.date,
                "num_games": daily_picks.num_games,
                "picks": picks_list,
                "premium_count": daily_picks.premium_count,
                "high_count": daily_picks.high_count,
                "standard_count": daily_picks.standard_count,
                "sportsbook_count": daily_picks.sportsbook_count,
                "derived_count": daily_picks.derived_count,
            })
    ...
    @app.route("/api/v2-system/backtest", methods=["POST"])
    def api_v2_system_backtest():
        """
        Run backtest on the V2 multi-model system.
        ...
        """
        try:
            from ..engine.multi_model_picker import run_backtest_multi_model
            ...
            # Run backtest
            result = run_backtest_multi_model(start_date, end_date, verbose=False, show_progress=False)
            ...
            response = {
                ...
                "picks_per_day": round(result.total_picks / max(result.days_tested, 1), 1),
                "by_tier": {
                    "premium": {
                        "picks": result.premium_picks,
                        "hits": result.premium_hits,
                        "rate": round(result.premium_hits / max(result.premium_picks, 1) * 100, 1),
                    },
                    "high": {
                        "picks": result.high_picks,
                        "hits": result.high_hits,
                        "rate": round(result.high_hits / max(result.high_picks, 1) * 100, 1),
                    },
                    "standard": {
                        "picks": result.standard_picks,
                        "hits": result.standard_hits,
                        "rate": round(result.standard_hits / max(result.standard_picks, 1) * 100, 1),
                    },
                },
                "by_line_source": {
                    "sportsbook": {
                        "picks": result.sportsbook_picks,
                        "hits": result.sportsbook_hits,
                        "rate": round(result.sportsbook_hits / max(result.sportsbook_picks, 1) * 100, 1),
                    },
                    "derived": {
                        "picks": result.derived_picks,
                        "hits": result.derived_hits,
                        "rate": round(result.derived_hits / max(result.derived_picks, 1) * 100, 1),
                    },
                },
                "by_pattern": {},
                "by_prop_type": {
                    "PTS": {
                        "picks": result.pts_picks,
                        "hits": result.pts_hits,
                        "rate": round(result.pts_hits / max(result.pts_picks, 1) * 100, 1),
                    },
                    "REB": {
                        "picks": result.reb_picks,
                        "hits": result.reb_hits,
                        "rate": round(result.reb_hits / max(result.reb_picks, 1) * 100, 1),
                    },
                    "AST": {
                        "picks": result.ast_picks,
                        "hits": result.ast_hits,
                        "rate": round(result.ast_hits / max(result.ast_picks, 1) * 100, 1),
                    },
                },
                "by_direction": {
                    "OVER": {
                        "picks": result.over_picks,
                        "hits": result.over_hits,
                        "rate": round(result.over_hits / max(result.over_picks, 1) * 100, 1),
                    },
                    "UNDER": {
                        "picks": result.under_picks,
                        "hits": result.under_hits,
                        "rate": round(result.under_hits / max(result.under_picks, 1) * 100, 1),
                    },
                },
            }
            ...
            for pattern, stats in result.pattern_stats.items():
                response["by_pattern"][pattern] = {
                    "picks": stats["picks"],
                    "hits": stats["hits"],
                    "rate": round(stats["hits"] / max(stats["picks"], 1) * 100, 1),
                }
