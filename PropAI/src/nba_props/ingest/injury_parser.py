...
class InjuryEntry:
    ...
    minutes_limit: Optional[float] = None  # V19.3: Detected minutes restriction (e.g., 28.0)
...
def _extract_minutes_limit(reason: str) -> Optional[float]:
    """
    V19.3: Extract minutes restriction from injury report reason text.
    
    NBA injury reports and news often mention minutes restrictions like:
    - "minutes restriction"
    - "limited to 28 minutes"
    - "minute limit"
    - "minutes limit 30"
    - "returning - 25 min restriction"
    
    Returns the numeric minutes limit if found, or a default estimate
    if a restriction is mentioned but no specific number is given.
    """
    reason_lower = reason.lower()
    
    # Pattern 1: Explicit numeric limit — "limited to X minutes", "X min restriction"
    numeric_patterns = [
        r'(?:limited\s+to|limit(?:ed)?|restriction)\s*(?:of\s+)?(\d{1,2})\s*(?:min|minutes)',
        r'(\d{1,2})\s*(?:min|minutes?)\s*(?:restriction|limit)',
        r'minutes?\s*(?:restriction|limit)\s*(?:of\s+)?(\d{1,2})',
    ]
    for pattern in numeric_patterns:
        m = re.search(pattern, reason_lower)
        if m:
            try:
                limit = float(m.group(1))
                if 10 <= limit <= 38:  # Sanity check
                    return limit
            except (ValueError, IndexError):
                pass
    
    # Pattern 2: General restriction mention without a number
    restriction_phrases = [
        "minutes restriction",
        "minute restriction",
        "minutes limit",
    ...
    for phrase in restriction_phrases:
        if phrase in reason_lower:
            return 25.0  # Default estimate: ~25 min restriction
...
def parse_injury_report_text(text: str) -> ParsedInjuryReport:
    ...
    def parse_player_entry_from_text(text: str, team_name: str) -> Optional[InjuryEntry]:
        ...
        minutes_limit = _extract_minutes_limit(reason_part)
...
def _parse_player_from_remaining(
    ...
    minutes_limit = _extract_minutes_limit(reason)
