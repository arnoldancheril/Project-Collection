...
def ingest_boxscore_file(conn, source_file: Path) -> int:
    ...
    # =========================================================================
    # After ingesting a new game, scan for any players whose team changed.
    # This auto-populates the player_trades table without manual CLI entry.
    try:
        from ..engine.trade_tracker import auto_detect_trades_from_boxscores
        auto_detect_trades_from_boxscores(
            conn,
            since_date=parsed.game_date,  # Only scan this game's date range
            verbose=False,
        )
    except Exception:
        pass  # Don't fail the ingest if trade detection has an issue
