...
def get_team_usage_profiles(
    ...
    V19.1 TRADE-AWARE: Can filter by date range so we only look at
    games AFTER a trade deadline. This prevents departed players from
    being included in usage calculations, and only counts players
    ...
    Args:
        ...
        as_of_date: Only consider games before this date (walk-forward)
        since_date: Only consider games AFTER this date (post-trade filter)
    ...
    date_params: list = []
    if as_of_date:
        date_filters += " AND g.game_date < ?"
        date_params.append(as_of_date)
    if since_date:
        date_filters += " AND g.game_date >= ?"
        date_params.append(since_date)
    ...
    rows = conn.execute(
        ...
        """,
        (*team_ids, *date_params, min_games),
    ).fetchall()
...
def calculate_usage_redistribution(
    ...
    V19.1: Now accepts as_of_date and since_date for trade-aware filtering.
    ...
    """
    profiles = get_team_usage_profiles(
        conn, team_abbrev, min_games,
        as_of_date=as_of_date, since_date=since_date,
    )
