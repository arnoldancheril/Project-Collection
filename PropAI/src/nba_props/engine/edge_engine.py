...
def get_projection_with_context(
    ...
    # V19.2: Get trade info for team-aware minutes projection
    current_team_id = None
    trade_date = None
    try:
        from .trade_tracker import get_player_trade_info_by_id
        trade_info = get_player_trade_info_by_id(conn, player_id)
        if trade_info:
            current_team_id = conn.execute(
                "SELECT id FROM teams WHERE name LIKE ?",
                (f"%{trade_info.to_team}%",)
            ).fetchone()
            if current_team_id:
    ...
    projection = project_player_minutes_first(
        conn, player_id, game_context,
