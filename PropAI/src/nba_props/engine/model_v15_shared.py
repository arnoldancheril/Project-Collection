"""
Model V15 Shared Utilities
===========================

Common functions, data classes, and utilities shared between:
- Model V15 General (Over/Value focused + General UNDER suggestions)
- Model V15 Under (UNDER specialized - placeholder for future development)

This module provides:
1. Player statistics loading with comprehensive windows
2. Sportsbook line fetching with intelligent fallback to derived lines
...
KEY DESIGN PRINCIPLES (Learned from V2-V14):
--------------------------------------------
1. HYBRID LINE APPROACH: Use sportsbook lines when available, projections when not
   - Track line source for all picks
   - Separate metrics for sportsbook vs derived line picks
...
2. PATTERN-BASED PICKS ONLY:
   ...
   - Cold Bounce (84.6% in V14) is the gold standard for OVER
   
3. HONEST REPORTING:
   - Track and report sportsbook vs derived hit rates separately
   - This addresses the "Derived Line Fallacy" that inflated prior model metrics

4. STRATEGIC DIRECTION:
   - PTS: UNDER preferred (63.9% vs 48.3% OVER from RCM)
   - REB: Both directions (~59% each)
   - AST: High bar (8.5+ avg only, too volatile otherwise ~54%)

5. DEFENSE INTEGRATION:
   - Elite defense (top 5 DVP) is strong UNDER signal (62.2%)
   - Combine with cold streak for highest confidence
...
ELITE_DEFENSE_RANK = 3      # Top 3 = elite defense (changed from 5 based on backtest - rank 4-5 was 27%)
...
MIN_PROP_AVERAGES = {
    ...
    'ast': 8.5,     # Min 8.5 APG for AST picks (very high bar - V9/V10 learnings)
}
...
@dataclass
class PlayerStatsV15:
    """
    Comprehensive player statistics for Model V15.
    ...
    def get_projection(
        ...
        Default weights (validated from V14 backtesting):
...
def get_line(
    ...
    This is the KEY FUNCTION for hybrid line handling per V10/V14 design.
...
def detect_cold_bounce_pattern(
    ...
    This is the BEST OVER pattern - 84.6% hit rate in V14 testing!
...
def detect_hot_sustained_pattern(
    ...
    This pattern is DISABLED by default in V15.
