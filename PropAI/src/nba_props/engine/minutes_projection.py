...
def project_minutes(
    ...
    minutes_limit: Optional[float] = None,  # V19.3: Known minutes restriction from injury report
) -> MinutesProjection:
    """
    ...
    # 6. V19.3: Known minutes restriction from injury report or team management
    if minutes_limit is not None and minutes_limit > 0:
        if adjusted_minutes > minutes_limit:
            old_mins = adjusted_minutes
            adjusted_minutes = minutes_limit
            pct_change = round((minutes_limit - old_mins) / old_mins * 100, 1)
            adjustments["minutes_limit"] = pct_change
            warnings.append(
                f"⚠️ Minutes restriction: capped at {minutes_limit:.0f} min "
                f"(was {old_mins:.1f})"
            )
...
def project_full_stat_line(
    ...
    mins_proj = project_minutes(
        ...
        minutes_limit=game_context.get("minutes_limit"),  # V19.3
    )
...
def project_player_minutes_first(
    ...
    V19.2: Trade-aware — if a player has been traded, only use games from
    their current team to avoid mixing old-team context.
    ...
    Args:
        ...
        current_team_id: If set, only use games where player played for this team
        trade_date: If set, only use games on or after this date
    ...
    # Build query with optional trade-aware filters
    query = """
        SELECT 
            g.game_date,
    ...
    # V19.2: Filter to current team only (for traded players)
    if current_team_id is not None:
        query += " AND b.team_id = ?"
        params.append(current_team_id)
    
    # V19.2: Filter to games after trade date
    if trade_date is not None:
        query += " AND g.game_date >= ?"
        params.append(trade_date)
    
    query += " ORDER BY g.game_date DESC LIMIT ?"
    params.append(games_lookback)
    
    logs = conn.execute(query, params).fetchall()
    
    if not logs:
        # If team filter returned nothing, fall back to unfiltered
        # (player may not have played for new team yet)
        if current_team_id is not None or trade_date is not None:
            logs = conn.execute(
                """
                SELECT 
                    g.game_date,
            ...
            if logs:
                game_context.setdefault("warnings_extra", []).append(
                    "Using pre-trade games — no new-team data yet"
                )
    ...
    # V19.2: Add trade-related warnings
    if current_team_id is not None and len(player_games) < MIN_GAMES_REQUIRED:
        game_context.setdefault("warnings_extra", []).append(
            f"Only {len(player_games)} new-team games for minutes projection"
        )
    
    result = project_full_stat_line(
        player_id=player_id,
        player_name=player_name,
        team_abbrev=team_abbrev,
        player_games=player_games,
    ...
    # Add any extra warnings
    if result and game_context.get("warnings_extra"):
        result.warnings.extend(game_context["warnings_extra"])
