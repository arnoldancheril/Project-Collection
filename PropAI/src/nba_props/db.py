...
def init_db(db_path: Path) -> None:
    ...
    try:
        ...
        # Initialize trade deadline tracking tables
...
def _init_trade_tables(conn: sqlite3.Connection) -> None:
    """Initialize trade deadline tracking tables if they don't exist."""
    try:
        from .engine.trade_tracker import TRADE_SCHEMA_SQL
        conn.executescript(TRADE_SCHEMA_SQL)
        conn.commit()
    except Exception:
        pass  # Trade module may not be importable in all contexts
