...
1. Go to `/paste` → Web Scraping section
2. Select date and click "🏀 Fetch Matchups"
3. Review scraped matchups and click "💾 Save to Database"
...
**Primary Source:** ESPN Web Scraper (https://www.espn.com/nba/injuries)  
**Secondary Source:** NBA Official Injury Report (manual paste)  
**Scraper:** `src/nba_props/ingest/web_scraper.py`  
...
**From ESPN Scraper (ScrapedInjury):**
```python
@dataclass
class ScrapedInjury:
    team_name: str      # "Phoenix Suns"
    player_name: str    # "Kevin Durant"
    position: str       # "SF"
    status: str         # "Out", "Day-To-Day"
    est_return_date: str # "Feb 10"
    comment: str        # "Calf strain"
```

**From Manual Parser (InjuryEntry):**
```python
@dataclass
class InjuryEntry:
...
#### Status Mapping (ESPN → Standard)

| ESPN Status | Mapped Status | Model Treatment |
|-------------|---------------|-----------------|
| Out | OUT | Removed from projections, usage redistributed |
| Suspension | OUT | Removed from projections |
| Injured Reserve | OUT | Removed from projections |
| Day-To-Day | QUESTIONABLE | Flagged, projections adjusted |
| Questionable | QUESTIONABLE | May play, uncertain |
| Doubtful | DOUBTFUL | Treated as OUT (~25% chance to play) |
| Probable | PROBABLE | Normal projections with slight caution |

#### How to Add Injury Data

**Option 1: ESPN Web Scraper (Recommended)**
1. Go to `/paste` → Web Scraping section
2. Click "🏥 Fetch Injuries" to scrape from ESPN
3. Review scraped injuries displayed on screen
4. Click "💾 Save to Database" to store

**Option 2: Manual Paste**
1. Go to `/paste` → Injury Report section
2. Paste NBA official injury report
...
#### Model V16 (Recommended - 72.4% Hit Rate)

```bash
# Using model_v16 (current production)
python -c "
from src.nba_props.engine.model_v16_general import run_backtest_v16_general

results = run_backtest_v16_general(
...
print(f'By Pattern: {results[\"by_pattern\"]}')
"
```
...
4. **Line Source Analysis (Model V16):**
   - ✅ Hit rate with actual sportsbook lines
   - ✅ Hit rate with derived lines (stricter thresholds)
   - ✅ Hybrid line handling validation

---

## Model V16 Backtest Results (Reference)

The current production model (V16.1) achieved:
...
| REB OVER | 61.1% (11/18) |
| Cold Bounce Pattern | 76.9% (30/39) |
| B2B Fatigue Pattern | 75.0% (24/32) |
| Elite Defense Factor | 67.9% (38/56) |

Key insight: **Fewer, higher-quality picks (127 vs 345) outperform many marginal picks.**

See `MODEL_V16.md` for complete details.
...
| **Sportsbook Lines** | **Model V16 hybrid lines** | **Daily (pre-game)** |
| Salaries | Role identification | Yearly |

By maintaining fresh data and running regular backtests, you can achieve **70-75%+ hit rates** on high-confidence picks with Model V16.
